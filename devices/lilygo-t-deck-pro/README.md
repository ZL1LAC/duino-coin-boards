...
2. Copy `devices/lilygo-t-deck-pro/TestConfig.h.example` → `TestConfig.h`
3. Sync:
   ```powershell
...
When that looks good, move to the full miner: `Settings.h` + `.\scripts\sync-device.ps1 lilygo-t-deck-pro miner` → `ESP_Code/ESP_Code.ino`.
...
| Mining OK, no display | `Settings.h` still set for another device — run `.\scripts\sync-device.ps1 lilygo-t-deck-pro miner` |
