...
1. Copy `devices/lilygo-t-deck/TestConfig.h.example` → `TestConfig.h`
2. Sync:
   ```powershell
...
When that looks good: `Settings.h` + `.\scripts\sync-device.ps1 lilygo-t-deck miner` → `ESP_Code/ESP_Code.ino`.
